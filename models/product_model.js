...
var product = {
  addProduct: function (item, filename, callback) {
    return db.query(
      "insert into product_tbl (p_name,p_price,p_dis,p_stock,p_disc_price,p_qty,p_unit,p_ben,p_usage,p_img,fk_sct_id) values (?,?,?,?,?,?,?,?,?,?,?)",
      [
        item.p_name,
        item.p_price,
        item.p_dis,
  ...
  updateProduct: function (item, callback) {
    return db.query(
      "update product_tbl set p_name=?,p_price=?,p_qty=?,p_stock=?,p_dis=?,p_unit=?,p_usage=?,p_disc=?,fk_sct_id=? where p_id=?",
      [
        item.p_name,
        item.p_price,
        item.p_qty,
  ...
  updateImage: function (p_id, filename, callback) {
    if (filename != null) {
      return db.query(
        "update product_tbl set p_img=? where p_id=?",
        [filename, p_id],
        callback
      );
    }
  },
  getMultipleProducts: function (item, callback) {
    return db.query(
      "select * from product_tbl where p_id in (?)",
      [item],
      callback
    );
  },
